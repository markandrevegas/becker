...
<template>
	<div>
		<header :class="props.isScrolled ? 'bg-abyssal/40 text-white backdrop-blur-xl' : 'text-abyssal backdrop-blur-md dark:text-yellow-50/90'" class="fixed right-0 top-0 z-40 flex h-[70px] w-full items-center justify-end px-4 transition-colors duration-300">
			<button class="text-2xl uppercase tracking-tighter" :aria-expanded="isOpen" aria-haspopup="true" @click="toggleMenu">
		...
		<nav class="fixed right-0 top-[70px] z-50 h-full w-3/4 bg-white text-abyssal dark:bg-abyssal dark:text-white transition-transform duration-300" :class="isOpen ? 'translate-x-0' : 'translate-x-full'">
			<div class="flex h-full flex-col">
				<div class="hidden fixed right-0 top-0 z-10 flex h-[70px] w-full items-center justify-between px-4">
					<ColorModeToggle />
				</div>
